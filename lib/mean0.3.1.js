...
Meanio.prototype.Module = function(name) {
	...
	this.aggregateAsset = function(type, path, options) {
		aggregate(type, this.name + '/public/assets/' + type + '/' + path, options);
	};
...
function enableModules() {
	events.on('modulesFound', function() {
		...
		for (var name in modules) {
			require(process.cwd() + '/node_modules/' + name + '/app.js');
		}
...
function aggregate(ext, path, options) {
	...
	events.on('modulesFound', function() {
		...
		for (var name in modules) {
			readFiles(ext, process.cwd() + '/node_modules/' + name + '/public/');
		}
	});
