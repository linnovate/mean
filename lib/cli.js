  ...
  request(options, function(error, response, body) {
    if (!error && (response.statusCode === 200 || response.statusCode === 201)) {
      ...
      results.forEach(function(pkg) {
        ...
        console.log(chalk.green(pkg.obj.name) + ' ' + pkg.obj.description);
...
exports.install = function(module, options) {
  requiresRoot(function() {
    ...
    getPackage(module, function(data) {
      ...
      console.log('git clone ' + (options.tag ? '--branch ' + options.tag : '') + ' ' + data.repo + ' node_modules/' + data.name);

      shell.exec('git clone ' + (options.tag ? '--branch ' + options.tag : '') + ' ' + data.repo + ' node_modules/' + data.name, function(code) {
