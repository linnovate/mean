...
exports.addRole = function(email, role, options) {
  requiresRoot(function() {
    console.log('  Adding role `' + role + '` to user `' + email + '`'.info);
...
exports.removeRole = function(email, role, options) {
  requiresRoot(function() {

    console.log('  Removing role `' + role + '` from user `' + email + '`'.info);
    var config = require(process.cwd() + '/server/config/env/' + options.env + '.js');
