...
var whoami = exports.whoami = function(callback) {
  ...
  if (token) {
    ...
    var options = {
      uri: 'http://127.0.0.1:3000/whoami',
      method: 'GET',
      headers: {
        'authorization': token
      }
  ...
  loadPackageJson('./package.json', function(err, pck) {
    ...
    checKeys(function(keys) {
      ...
      shell.exec('git init', function(code) {
        ...
        shell.exec('git add .', function(code) {
          if (code) return console.log('Error: failed to add files to git');

          shell.exec('git commit -am "publish"', function(code) {
            if (code) return console.log('Error: failed to commit files to git');

            // shell.exec('git remote add origin ' + source, function(code) {
            //   console.log('4 ' + code);

            //   if (!code) {
            //     shell.exec('git push -u origin master', function(code) {
            //       console.log('5 ' + code);
            //       if (!code) {
            //         callback('Project created successfully')
            //       }
            //     });
            //   }
            // });
...
function publishPackage(data, callback) {
  var token = readToken();
  var options = {
    uri: 'http://127.0.0.1:3000/packages/publish',
    method: 'POST',
    form: data,
    headers: {
      'Content-Type': 'multipart/form-data',
