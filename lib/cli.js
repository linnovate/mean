...
var whoami = exports.whoami = function(callback) {
  ...
  if (token) {
    ...
    var options = {
      uri: 'https://network.mean.io/whoami',
      method: 'GET',
      headers: {
        'authorization': token
      }
...
exports.register = function() {
  ...
  prompt.get({
    ...
    function(err, result) {
      ...
      var options = {
        uri: 'https://network.mean.io/user',
        method: 'POST',
        form: result,
        headers: {
  ...
  prompt.get({
    ...
    function(err, result) {
      ...
      if (token) {
        ...
        var options = {
          uri: 'https://network.mean.io/keys',
          method: 'POST',
          form: keyData,
          headers: {
            'Content-Type': 'multipart/form-data',
                ...
                shell.exec('git remote add meanio ' + data.repo, function(code) {
                  ...
                  if (!code) {
                    shell.exec('git push -u meanio master', function(code) {
                      ...
                      if (!code) {
                        console.log('Publish Success! ' + data.name + ' distribution (' + data.index + ') ' + data.version);
                      }
                    });
                  }
                });
...
function publishPackage(data, callback) {
  ...
  var options = {
    uri: 'https://network.mean.io/packages/publish',
    method: 'POST',
